...
import com.bdas2.app.repository.modelRepositories.ZastavkaRepository;
...
@CrossOrigin
@RestController
@RequestMapping("/Zastavka")
public class ZastavkaController {
    public final ZastavkaRepository repo;

    public ZastavkaController(ZastavkaRepository repo) {
        this.repo = repo;
    }
    ...
    @PostMapping("/create")
    @ResponseBody
    public ResponseEntity create(@RequestBody Zastavka body) {
        try {
    ...
    @PostMapping("/update")
    public ResponseEntity update(@RequestParam Integer id, @RequestBody Zastavka body) {
        try {
